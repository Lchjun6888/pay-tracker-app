...
const WalletView = () => {
    ...
    const [banks, setBanks] = useState([
        { id: 1, name: 'KB국민은행', desc: '저축예금 •••• 8829', amount: 4200000, iconBg: 'bg-amber-100', iconColor: 'text-amber-600', icon: 'fa-building-columns' },
        { id: 2, name: '신한은행', desc: '입출금통장 •••• 1042', amount: 8250000, iconBg: 'bg-indigo-100', iconColor: 'text-indigo-600', icon: 'fa-credit-card' }
    ]);
    ...
    const handleWithdraw = () => {
        const amountStr = prompt('출금할 금액을 입력하세요 (원):');
        if (!amountStr) return;
        ...
        setRecentActivities(prev => [{
            id: Date.now(), type: 'out', title: '출금', subtitle: '직접 출금 신청', amount: amount, date: 'TODAY'
        }, ...prev]);
    ...
    const handleAddFunds = () => {
        ...
        setBalance(prev => prev + amount);
        setRecentActivities(prev => [{
            id: Date.now(), type: 'in', title: '충전', subtitle: '계좌 간편 충전', amount: amount, date: 'TODAY'
        }, ...prev]);
        alert(`${amount.toLocaleString()}원이 충전되었습니다.`);
    };

    const handleLinkAccount = () => {
        const bankName = prompt('연동할 은행 또는 카드사를 입력하세요 (예: 카카오뱅크):');
        if (!bankName) return;
        const newBank = {
            id: Date.now(),
            name: bankName,
        ...
        alert(`${bankName} 계좌가 성공적으로 연동되었습니다!`);
    };
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                        <div>
                            <h1 className="text-4xl font-extrabold text-slate-900 dark:text-white font-mono">{balance.toLocaleString()}원</h1>
                        ...
                        <div className="flex gap-3 w-full md:w-auto">
                            <button
                                ...
                                className="flex-1 md:flex-none bg-green-500 hover:bg-green-600 text-white font-bold px-5 py-3 rounded-xl shadow-sm transition-all flex items-center justify-center gap-2 active:scale-95">
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {banks.map(bank => (
                            <div key={bank.id} className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 hover:shadow-md transition-shadow cursor-pointer">
                                <div className="flex justify-between items-start mb-4">
                                    <div className={`w-10 h-10 ${bank.iconBg} dark:bg-opacity-20 rounded-lg flex items-center justify-center ${bank.iconColor}`}>
                                        <i className={`fas ${bank.icon}`}></i>
                                    </div>
                                    <i className="fas fa-ellipsis-v text-slate-300 hover:text-slate-500 cursor-pointer"></i>
                                </div>
                                <h4 className="font-bold text-slate-900 dark:text-white">{bank.name}</h4>
                                <p className="text-slate-500 text-sm mb-2">{bank.desc}</p>
                                <p className="font-mono font-bold text-xl text-slate-900 dark:text-white">{bank.amount.toLocaleString()}원</p>
                            </div>
                        ))}
                        ...
                        <div
                            ...
                            className="border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-2xl flex flex-col items-center justify-center p-5 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer text-slate-400 hover:text-slate-600 dark:hover:text-white min-h-[160px] active:scale-95">
                    ...
                    <div className="space-y-2">
                        {['출금은 얼마나 걸리나요?', 'Open Banking은 안전한가요?', '결제 분쟁 정책'].map((q, i) => (
                            <button
                                key={i}
                                onClick={() => alert(`"${q}"\n\n이 기능은 데모 버전입니다.`)}
                                className="w-full flex items-center justify-between text-sm text-slate-600 dark:text-slate-400 hover:text-primary-500 py-2 border-b border-slate-100 dark:border-slate-700 last:border-0"
                            >
                                <span>{q}</span>
